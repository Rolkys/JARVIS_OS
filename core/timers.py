...
class TimerManager:
    ...
    def __init__(self, speaker=None, sounds=None):
        ...
        logger.info("Gestor de temporizadores iniciado")
    ...
    def cancel_timer(self) -> str:
        """Cancela el ultimo temporizador activo"""
    ...
    def cancel_alarm(self) -> str:
        """Cancela la ultima alarma activa"""
    ...
    def list_timers(self) -> str:
        ...
        respose = "Temporizadores activos: "
        for i, timer in enumerate(active):
            remaining = timer['end_time'] - datetime.now()
            if remaining.total_seconds() > 0:
                respose += f"{i+1}. {time['label']} (faltan {self._format_timedelta(remaining)}),"

        return respose.strip(' , ')

    def list_alarms(self) -> str:
        """Listar alarmas programadas"""
    ...
    def _timer_thread(self, timer):
        """Espera a que termine el temporizador"""
        while not timer['done']:
            reamining = (timer['end_time'] - datetime.now()).total_seconds()
            if reamining <= 0:
                timer['done'] = True
                self._tirgger_time(timer)
                break
            time.sleep(0.5)
    
    def _check_alarms(self):
        """Verifica alarmas periodicamente"""
        while self.running:
            now = datetime.now()
            for alarm in self.alarms:
    ...
    def _trigger_timer(self, timer):
        """Acciones al terminar temporizado"""
    ...
    def parse_time(self, command:str) -> str:
        """Extrae tiempo en sefundos con un comando"""
    ...
    def parse_alarm_time(self, command: str) -> Optional[tuple]:
        """Extrae hora y mintos de una alarma"""
